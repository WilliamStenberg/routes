...
def smallest_map_enclosing(sess, box) -> Optional[Map]:
    PRBB = PaddedRouteBoundingBox
    stmt = (
        select(Map)
        .join(PRBB)
        .where(PRBB.west <= box.west)
        .where(PRBB.east >= box.east)
        .where(PRBB.south <= box.south)
        .where(PRBB.north >= box.north)
        .order_by((PRBB.north - PRBB.south) * (PRBB.east - PRBB.west))
    )
